...
//   // Simple clean state for dropdown sorting selection
//   const [orderSortOption, setOrderSortOption] = useState("latest");
...
//   // Sorting computation using standard Mongo _id string timestamp values
//   const getSortedOrders = () => {
//     const sorted = [...orders];
//     if (orderSortOption === "latest") {
//       return sorted.sort((a, b) => b._id.localeCompare(a._id));
//     } else if (orderSortOption === "oldest") {
...
//               { id: "products", label: "Products List", count: productsList.length },
...
//                       <tr>
//                         <td colSpan="4" className="text-center py-10 text-slate-500">No products found.</td>
//                       </tr>
//                     ) : (
//                       productsList.map(p => (
...
const Admin = () => {
  ...
  // ================= NEW STATES: SORTING, SEARCH, & PAGINATION =================
  ...
  // Reset pagination pages when switching tabs to avoid empty tables
  useEffect(() => {
    setOrdersPage(1);
    setProductsPage(1);
    setUsersPage(1);
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      setProductsList(prodResult?.data?.products || []);
      setUsersList(usersResult?.data?.users || usersResult?.data || []);
  ...
  // ================= CORE DATA LOGIC: PROCESSING, SORTING & FRONTEND PAGINATION =================

  // 1. Process Orders Data
  const getProcessedOrders = () => {
    let result = [...orders];
    if (ordersSort === "latest") {
  ...
  const totalOrdersPages = Math.ceil(processedOrders.length / ITEMS_PER_PAGE) || 1;
  const paginatedOrders = processedOrders.slice((ordersPage - 1) * ITEMS_PER_PAGE, ordersPage * ITEMS_PER_PAGE);

  // 2. Process Products Data
  const getProcessedProducts = () => {
    let result = [...productsList];
    if (productsSort === "latest") {
      result.sort((a, b) => (b._id || b.productId || "").localeCompare(a._id || a.productId || ""));
    } else {
      result.sort((a, b) => (a._id || a.productId || "").localeCompare(b._id || b.productId || ""));
    }
    return result;
  };
  const processedProducts = getProcessedProducts();
  const totalProductsPages = Math.ceil(processedProducts.length / ITEMS_PER_PAGE) || 1;
  const paginatedProducts = processedProducts.slice((productsPage - 1) * ITEMS_PER_PAGE, productsPage * ITEMS_PER_PAGE);

  // 3. Process Users Data (With Email Filter & Dropdown Sorting)
  const getProcessedUsers = () => {
    let result = [...usersList];
    if (userSearchQuery.trim() !== "") {
      result = result.filter(u => u.email?.toLowerCase().includes(userSearchQuery.toLowerCase()));
    }
    if (usersSort === "latest") {
      result.sort((a, b) => (b._id || "").localeCompare(a._id || ""));
    } else {
  ...
  const totalUsersPages = Math.ceil(processedUsers.length / ITEMS_PER_PAGE) || 1;
  const paginatedUsers = processedUsers.slice((usersPage - 1) * ITEMS_PER_PAGE, usersPage * ITEMS_PER_PAGE);


  // Shared Pagination UI Renderer Block
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-800 text-sm">
                    {paginatedUsers.length === 0 ? (
                      <tr>
                        <td colSpan="3" className="text-center py-10 text-slate-500">No profile signatures active matching criteria.</td>
                      </tr>
                    ) : (
