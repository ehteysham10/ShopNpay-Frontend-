...
const Admin = () => {
  ...
  const proceedOrder = async (orderItem) => {
    ...
    // Updated to use native structural database document _id identifier
    const res = await fetch(`${API_URL}/orders/${orderItem._id}/status`, {
      method: "PATCH",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`
        ...
        <div>
          ...
          <nav className="space-y-1.5">
            {[
              ...
              >
                <span>{tab.label}</span>
                <span className={`text-xs px-2 py-0.5 rounded-md ${activeTab === tab.id ? "bg-[#705410] text-white" : "bg-[#FAF7F2] text-[#7A6A55] border"
                    ...
                    {usersList.length === 0 ? (
                      ...
                      usersList.map(u => (
                        <tr key={u._id} className="hover:bg-[#FAF7F2]/40 transition duration-150">
                          ...
                          <td className="py-4 px-6">
                            <span className={`inline-flex px-2.5 py-0.5 rounded text-xs font-semibold uppercase tracking-wide border ${u.role === "admin"
